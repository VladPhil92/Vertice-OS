...
function formatDate(iso: string): string {
  ...
  return `${String(d.getDate()).padStart(2, '0')} ${ES_MONTHS[d.getMonth()]} ${d.getFullYear()}`
}
  ...
  return (
    <article className="flex flex-col gap-4 border border-border bg-surface p-6 transition-all hover:border-border-active">
      <div className="flex items-center justify-between">
        ...
        <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-gold border border-gold/20 px-2 py-0.5">
      ...
      <p className="font-mono text-[11px] leading-relaxed text-secondary">{preview}</p>
      ...
      <div className="mt-auto flex items-center justify-between pt-2 border-t border-border">
        <button
          ...
          aria-label={`Avalar propuesta: ${proposal.title}`}
        ...
        >
          <ThumbsUp size={13} strokeWidth={1.5} className={endorsing ? 'opacity-50' : ''} />
        ...
        <time dateTime={proposal.created_at} className="font-mono text-[10px] text-tertiary">
      ...
      <main className="mx-auto max-w-5xl px-6 py-10">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="font-display text-3xl font-bold text-primary">Propuestas ciudadanas</h1>
        ...
        <div className="mb-8 flex flex-wrap items-center gap-3">
          ...
          <select value={filterScope} onChange={e => setFilterScope(e.target.value)} className={SELECT_CLASS}>
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center gap-5 py-24">
            ...
            <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-tertiary">Cargando propuestas…</span>
          </div>
        )}
          ...
          <div className="border border-border bg-surface px-6 py-16 text-center">
            ...
            <p className="mt-3 font-mono text-sm text-tertiary">
              {proposals.length === 0 && (
                <>
                  Sé el primero —{' '}
                  <Link href="/dashboard/proposals/new" className="text-gold hover:underline">crea una propuesta</Link>.
                </>
              )}
            </p>
          </div>
