...
const TABS = [
  ...
  { href: '/dashboard/community/actions', label: 'Acciones cívicas', icon: ClipboardCheck, exact: false },
          ...
          {TABS.map(({ href, label, icon: Icon, exact }) => {
            ...
            return (
              <Link
                ...
                href={href}
                className={active
                  ? 'inline-flex min-h-9 shrink-0 items-center gap-2 rounded-full bg-[#0A2A66] px-3.5 py-2 text-[10px] font-extrabold text-white'
                  : 'inline-flex min-h-9 shrink-0 items-center gap-2 rounded-full border border-[#DCE5EF] bg-white px-3.5 py-2 text-[10px] font-extrabold text-[#607087]'}
              >
                <Icon size={13} /> {label}
              </Link>
            )
