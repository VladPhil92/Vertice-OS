...
-- en el instante debate→voting. eligible_voters en proposals se calcula como
-- el número de filas insertadas aquí en la misma transacción — no puede
-- desincronizarse del padrón nominal.
...
-- Reemplaza el patrón "enviar y olvidar" para mint de badges SBT y registro
-- de resultados de votación on-chain — ver apps/api/src/lib/jobs.ts.
CREATE TABLE jobs (
    id           SERIAL PRIMARY KEY,
    type         TEXT NOT NULL,
...
-- quién, qué hizo, sobre qué, cuándo, resultado. Solo INSERT desde la app.
