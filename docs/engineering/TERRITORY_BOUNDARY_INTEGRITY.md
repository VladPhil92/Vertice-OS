...
`GET /territories/resolve?lat=<lat>&lng=<lng>`
...
- `catalog_unavailable`: no synchronized boundary catalog is available;
- `unresolved`: a boundary catalog exists but no candidate can be resolved.
...
Territorial report analytics now include a `by_territory` projection grouped by the contribution's immutable `territory_code`. They do not group civic activity by the citizen's home territory.
...
A successful synchronization requires at least 500 persisted municipal/district boundaries. Partial writes never erase the previous valid geometry for municipalities that were not refreshed.

The catalog status reports whether enough boundaries exist for national point-in-polygon readiness. Repository tests use deterministic synthetic polygons and never require the external DANE service to be online.
...
Code, migration, deterministic integration tests and the synchronization mechanism can be certified in CI. **National polygon coverage is not certified until an authorized operator runs the production boundary synchronization and the status endpoint reports a sufficiently populated MGN catalog.**
...
4. verify `point_in_polygon_ready=true` and inspect counts/version;
